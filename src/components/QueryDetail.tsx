...
function QueryDetail() {
  ...
  const { state } = useLocation() as { state: { book?: Book } | null };
  const { dispatch } = useBookShelf();

  // 목록에서 넘어온 경우 router state 로 책 정보를 그대로 받음(추가 API 호출 불필요).
  // URL 직접 접근 시에는 ISBN 으로 정확히 1건만 조회.
  const [thisbook, setThisbook] = useState<Book | null>(state?.book ?? null);
  const [savedMsg, setSavedMsg] = useState("");

  useEffect(() => {
    if (thisbook || !itemID) return;

    let alive = true;
    (async () => {
      try {
        const isbn = decodeURIComponent(itemID).split(" ")[0];
        ...
        if (alive) setThisbook(data.documents[0] ?? null);
      } catch (err) {
  ...
  const addToShelf = (shelf: ShelfKey, label: string) => {
    if (!thisbook) return;
    dispatch({ type: "ADD", shelf, book: thisbook });
    setSavedMsg(`'${label}'에 담았어요!`);
  };

  if (!thisbook) {
    return (
      <div className="detail" style={{ minHeight: `calc(100vh - 73px)` }}>
        <p style={{ padding: "2rem", textAlign: "center" }}>책 정보를 불러오는 중입니다...</p>
      </div>
    );
  }
    ...
    <div className="detail" style={{ minHeight: `calc(100vh - 73px)` }}>
      ...
      <div className="content">
        ...
        <ul>
          ...
          <li>
            <a href={thisbook.url} target="_blank" rel="noreferrer">자세히 보기</a>
          </li>
        </ul>
        <div className="savebtns">
          ...
          <div className="btns">
            <button onClick={() => addToShelf(SHELVES.RED, "읽은 책")}>
            ...
            <button onClick={() => addToShelf(SHELVES.READING, "읽고 있는 책")}>
            ...
            <button onClick={() => addToShelf(SHELVES.WANNA, "읽고 싶은 책")}>
              <svg viewBox="0 0 49 55" fill="none" xmlns="http://www.w3.org/2000/svg">
