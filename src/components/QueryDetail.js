...
function QueryDetail() {
  ...
  const addlistREADING = () => {
    const userREADING = [{ READING: thisbook }];
    window.localStorage.setItem("READING", JSON.stringify(userREADING));
  }

  const addlistWANNA = () => {
    const userREADING = { WANNA: thisbook };
    window.localStorage.setItem("WANNA", JSON.stringify(userREADING));
  }
    ...
    <div className="detail" style={{ minHeight: `calc(100vh - 73px)` }}>
      ...
      <div className="content">
        ...
        <div className="savebtns">
          <h4>내 서재에 등록하기</h4>
          <div className="btns">
            {/* onClick={handleAddREDlist(thisbook)} */}
            <button>
